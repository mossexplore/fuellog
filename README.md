...
仪表盘以渐变主卡片突出平均油耗，并展示最近一箱油耗（含较上一箱的升降变化）、每公里油费、累计里程、累计油费、累计加油量和加油次数。下方趋势图可在油耗、油价、油费、里程四个指标间一键切换：油耗与油价为折线图，油费（每次加油费用）与里程（每次记录新增里程）为柱状图；支持按年份筛选（由近到远），点击图表中的数据位置即可查看该点具体数值。平均油耗按“两次加满之间的实际补油量”加权计算，避免非加满记录造成误差。
