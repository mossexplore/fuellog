...
- 数据导入导出：一键导出 UTF-8 BOM CSV，支持把本项目导出的 CSV 再导入回来，便于迁移、备份和恢复。
- 腾讯出行迁移：支持导入腾讯出行导出的历史油耗 XLSX；导入时会根据“最新油耗”连续重复值推断加满分段，组内最新一条标记为加满，其余记录标记为未加满，首条新车基线记录也按未加满处理。
- 批量清空：记录页提供“一键清空”，需要连续两次确认后才会删除全部记录和关联附件。
